...
class AdvancedCalibration:
    ...
    def detect_ellipse_contour(self, image: np.ndarray) -> Optional[Tuple[int, int, int, dict]]:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        ...
        height, width = image.shape[:2]
        min_area = (min(width, height) * 0.3) ** 2 * math.pi
        max_area = (min(width, height) * 0.6) ** 2 * math.pi
            ...
            if len(contour) >= 5:
                ...
                if min_area <= area <= max_area:
                    ...
                    if ma > 0 and mi > 0:
                        ...
                        score = (1 - eccentricity) * area_ratio
                        ...
                        if score > best_score:
                            ...
                            best_ellipse = (int(cx), int(cy), avg_radius, ellipse_data)
    ...
    def score_circle(self, image: np.ndarray, cx: int, cy: int, radius: int) -> float:
        height, width = image.shape[:2]

        center_score = 1.0 - (abs(cx - width/2) + abs(cy - height/2)) / (width + height)

        expected_radius = min(width, height) * 0.3
        size_score = 1.0 - abs(radius - expected_radius) / expected_radius
        ...
        edge_score = np.sum(overlap > 0) / (2 * math.pi * radius)
    ...
    def validate_dartboard(self, image: np.ndarray, center: Tuple[int, int], radius: int) -> Tuple[bool, float]:
        cx, cy = center
        height, width = image.shape[:2]

        if not (radius * 1.5 < cx < width - radius * 1.5 and radius * 1.5 < cy < height - radius * 1.5):
            return True, 0.6

        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

        roi_size = int(radius * 2.2)
        x1 = max(0, cx - roi_size // 2)
        y1 = max(0, cy - roi_size // 2)
        x2 = min(width, cx + roi_size // 2)
        y2 = min(height, cy + roi_size // 2)
    ...
    def calibrate_dartboard(self, image: np.ndarray, use_advanced: bool = True) -> CalibrationResult:
        ...
        for method in methods_to_try:
            ...
            for cx, cy, r in circles:
                score = self.score_circle(image, cx, cy, r)
                candidates.append((cx, cy, r, score, f"hough_{method}"))
        ...
        if use_advanced:
            ...
            if color_result:
                ...
                score = self.score_circle(image, cx, cy, r)
                candidates.append((cx, cy, r, score, "color_seg"))

            ellipse_result = self.detect_ellipse_contour(image)
            if ellipse_result:
                cx, cy, r, ellipse_data = ellipse_result
                score = self.score_circle(image, cx, cy, r)
                candidates.append((cx, cy, r, score, "ellipse"))
