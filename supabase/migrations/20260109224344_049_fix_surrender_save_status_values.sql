/*
  # Fix Surrender and Save Functions - Correct Status Values

  1. Changes
    - Update surrender_game to use correct status values from game_rooms
    - Update save_and_pause_game to use correct status values
    - Status values: 'waiting', 'in_progress', 'paused_disconnect', 'paused_mutual', 'completed', 'abandoned', 'forfeited'

  2. Notes
    - The functions were checking for wrong status values
    - Actual statuses in game_rooms are different from what was assumed
*/
...
BEGIN
  ...
  -- Get game details from game_rooms
  SELECT *
  INTO v_game
  FROM game_rooms
  WHERE id = p_game_id
    AND status NOT IN ('completed', 'abandoned', 'forfeited');
