/*
  # Fix Surrender and Save Game Functions

  1. Changes
    - Update surrender_game to use game_rooms instead of online_games
    - Update save_and_pause_game to use game_rooms instead of online_games
    - Fix table references to match actual schema

  2. Notes
    - The functions were referencing online_games but the actual table is game_rooms
    - This was causing the surrender and save buttons to fail
*/
...
BEGIN
  ...
  FROM game_rooms
  WHERE id = p_game_id
    AND status IN ('active', 'in_progress', 'paused');
